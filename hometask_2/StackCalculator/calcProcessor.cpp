...
bool CalcProcessor::isInteger(QChar const &x)
{
    return ((x >= QChar('0') && x <= QChar('9')) || (x == QChar('.')));
}

bool CalcProcessor::isOperator(QChar const &x)
{
    return (x == QChar('+') || x == QChar('*') || x == QChar('-') || x == QChar('/'));
}

float CalcProcessor::calcCurrent(const float &arg1, const float &arg2, const QChar &action)
{
